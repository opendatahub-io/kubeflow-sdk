...
def _build_callback_injection_code(callbacks: list) -> str:
    """Generate code that injects TrainingHubCallback instances into HuggingFace Trainer.

    Serializes each user callback class via inspect.getsource, then generates code that
    monkey-patches Trainer.__init__ to inject the adapted callbacks. The kubeflow SDK
    (with adapter classes) is already installed in the training pod.

    Args:
        callbacks: List of TrainingHubCallback subclass classes or instances.

    Returns:
        Python code string to prepend before user training code.
    """
    # Serialize each user callback class source
    callback_sources = []
    class_names = []
    for cb in callbacks:
        cls = cb if isinstance(cb, type) else type(cb)
...
from kubeflow.trainer.rhai.adapters.unsloth import UnslothCallbackAdapter
import transformers as _cb_transformers

# --- User callback definitions ---
{all_callback_code}

# --- Monkey-patch Trainer.__init__ to inject adapted callbacks ---
_cb_original_trainer_init = _cb_transformers.Trainer.__init__

def _cb_instrumented_trainer_init(self, *args, **kwargs):
    result = _cb_original_trainer_init(self, *args, **kwargs)
    _hub_callbacks = [{instantiation_lines}]
    for _hub_cb in _hub_callbacks:
        _adapter = UnslothCallbackAdapter(_hub_cb)
        self.add_callback(_adapter)
    print(f"[Kubeflow] Injected {{len(_hub_callbacks)}} TrainingHub callback(s)", flush=True)
    return result

_cb_transformers.Trainer.__init__ = _cb_instrumented_trainer_init
print("[Kubeflow] Callback injection configured", flush=True)
"""
...
def get_trainer_cr_from_transformers_trainer(
    ...
    # Inject user callbacks if provided
    if trainer.callbacks:
        callback_header = _build_callback_injection_code(trainer.callbacks)
        func_code = f"{callback_header}\n\n{func_code}"
